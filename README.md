...
Install it with pip (version up to 2.0):

```
pip install colifrapy
```

If you want to use the latest one (3.0) which is still in development :
...
Moreover, colifrapy is not a tyrant and does not force you to go its way. As such, every part of colifrapy can
...
Every bit of colifrapy, if you only need one, can be used as a standalone. Here they are:
...
Example of controller as it is generated by the scaffolder:
...
####Strings
Strings are externalized to enable you to quickly modify the strings if
...
class MyModel(Model):
	def test(self):
		...
		# will return True for yes and False for no
